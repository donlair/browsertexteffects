import { type Color, type GradientDirection, type EasingFunction, color, adjustBrightness } from "../types";
import { Gradient, coordKey } from "../gradient";
import type { Canvas } from "../canvas";
import { EffectCharacter } from "../character";
import { EventHandler } from "../events";
import { ParticleSystem } from "../particles";
import { makeEasing, outQuint, inCirc } from "../easing";
...
export class ThunderstormEffect {
  ...
  // Rain overlay
  private idleRainDrops: RainDrop[] = [];
  private activeRainDrops: RainDrop[] = [];
  private rainDelay = 0;
  ...
  private stormTick = 0;
  private strikeBranchChance = 0.05;
   ...
   */
  private _setupLightningStrike(branchNeighbor: OverlayChar | null = null): void {
    const { dims } = this.canvas;
    ...
    while (row >= dims.bottom) {
      ...
      if (!isFirstBranchSegment && Math.random() < this.strikeBranchChance
          && this.pendingStrikeChars.length < 100) {
    ...
    // Setup sparks at the impact point of this bolt
   ...
   */
  private _lightningStrike(): void {
    const { config } = this;

    this._setupLightningStrike();

    // Shared per-strike easing: makeEasing(0, 1.6, 1, random(-0.6, 0.4))
    const flashEase: EasingFunction = makeEasing(0, 1.6, 1, Math.random() - 0.6);

    // Gradients for bolt flash/fade
    ...
    // Create flash/fade scenes on ALL pending strike chars with shared easing
    ...
    // Apply the same flash easing to all text chars' flash scenes
  ...
  private _emitSparks(): void {
    ...
    for (const setup of this.pendingSparkSetups) {
      ...
      // Glow scene with inCirc easing (matching Python)
      ...
      // Path: arc to bottom via bezier control (matching Python)
      ...
      // Bezier control: midpoint column, random row (matching Python's bezier_control calculation)
      const bezierColumn = setup.spawnCol - Math.floor((setup.spawnCol - targetCol) / 2);
      const bezierControl = { column: bezierColumn, row: randInt(1, dims.top) };
  ...
  private _stepLightningStrike(): void {
    ...
    for (let i = 0; i < count; i++) {
      ...
      if (this.pendingStrikeChars.length === 0) {
        ...
        // Register strikeInProgress reset on the last revealed char's fade completion
        oc.ch.eventHandler.register("SCENE_COMPLETE", "fade", "CALLBACK", {
          callback: () => { this.strikeInProgress = false; },
          args: [],
        });
  ...
  private _rainTick(): void {
    ...
    for (let i = this.activeRainDrops.length - 1; i >= 0; i--) {
      const drop = this.activeRainDrops[i] as RainDrop;
      drop.col += drop.speed;
      drop.row -= drop.speed;
      if (drop.row < dims.bottom - 1) {
        // Recycle to idle pool
    ...
    if (this.rainDelay > 0) {
      ...
      for (let i = 0; i < count; i++) {
        ...
        const drop = this.idleRainDrops.pop() as RainDrop;
